...
define('OxiAddonsCustomData', $upload_dir . '/OxiAddonsCustomData/');

$CustomAddons = glob(OxiAddonsCustomData . '*', GLOB_ONLYDIR);
foreach ($CustomAddons as $link) {
    $folder = explode('/OxiAddonsCustomData/', $link);
    include_once( OxiAddonsCustomData . $folder[1] . '/' . $folder[1] . '.php');
}
...
function oxi_addons_home() {
    ...
    include oxi_addons_url . 'admin/class_shortcode_remote.php';
...
function oxi_addons_install() {
    ...
    $upload = wp_upload_dir();
    $upload_dir = $upload['basedir'];
    $OXI_dir = $upload_dir . '/OxiAddonsElements';
    if (!is_dir($OXI_dir)) {
        mkdir($OXI_dir, 0777);
    }
    $OXI_custom_dir = $upload_dir . '/OxiAddonsCustomData';
    if (!is_dir($OXI_custom_dir)) {
        mkdir($OXI_custom_dir, 0777);
    }
...
function OxiAddonsAdminajaxData() {
    ...
    if ($list_order == 'Installing Data') {
        ...
        $custom_check = sanitize_text_field($_POST['custom_check']);
        $tmpfile = download_url('https://shortcode-addons.com/ShortcodeElements/' . $list_items . '.zip', $timeout = 500);
        if (is_string($tmpfile)) {
            ...
            if ($list_items == 'elementor_templates') {
                if (!function_exists('is_plugin_active')) {
                    include_once( ABSPATH . 'wp-admin/includes/plugin.php' );
                }
                if (is_plugin_active('elementor/elementor.php')) {
                    (!empty($custom_check) ? $zip->extractTo(OxiAddonsCustomData) : $zip->extractTo(OxiAddonsElements));
                }
            } else {
                (!empty($custom_check) ? $zip->extractTo(OxiAddonsCustomData) : $zip->extractTo(OxiAddonsElements));
            }
