...
if (!empty($_POST['OxiAddonsElementsDLT']) && $_POST['OxiAddonsElementsDLT'] == 'Delete') {
    if (!wp_verify_nonce($nonce, 'Oxi-Addons-Delete-addons-Nonce')) {
        die('You do not have sufficient permissions to access this page.');
    } else {
        $folder = sanitize_text_field($_POST['OXIAddonsElements']);
        if ($folder != 'user_control') {
            $target_folder = OxiAddonsCustomData . $folder . '/';
            WP_Filesystem();
...
foreach ($DIRfiles as $value) {
    ...
    if (!empty($value)) {
        if (!empty($value) && count($file) == 2) {
            ...
            if (file_exists(OxiAddonsCustomData . $file[1] . '/version.php')) {
                $version = include_once OxiAddonsCustomData . $file[1] . '/version.php';
                if (is_array($version)) {
                    if (count($version) == 4 && $version[2] == true) {
                        $vs = $version;
                    }
                }
            }
            $customaddons[$file[1]] = array(
                'name' => $file[1],
                'homepage' => $vs[3],
                'version' => $vs[0],
                'control' => $vs[2]
...
if (!empty($_GET['oxitype']) && $_GET['oxitype'] == 'oxiprofile') {
        ...
        <div class="oxilab-admin-wrapper">
            <div class="oxi-addons-promote-header">
                ...
                <div class="oxi-addons-promote-header-button">
                    ...
                    <button type="button" class="oxi-btn oxi-btn-2" oxilink="https://www.shortcode-addons.com/elements/">Elements</button>
                </div>
            </div>
    ...
    $elements = SHORTCODEADDONS\Shortcode_Remote::shortcode_get_instance()->categories_list();
        ...
        <div class="oxi-addons-wrapper">   
            ...
            foreach ($elements[0] as $key => $element) {
                ...
                foreach ($element as $val) {
                    ...
                    foreach ($addonsfile as $importelements) {
                        if ($importelements == $val[0]) {
                            $valuetrue = 'true';
                        }
                    }
                    if ($valuetrue != 'true') {
                        $elementsnumber++;
                        $elementshtml .= '<div class="oxi-addons-shortcode-import"  oxi-addons-search="' . $val[0] . '" id ="' . $val[0] . '">
                                                    <a target="_blank" href="https://www.shortcode-addons.com/elements/' . str_replace('_', '-', $val[0]) . '">
                                                       <div class="oxi-addons-shortcode-import-top">
                                                              ' . oxi_addons_admin_font_awesome('oxi-' . $val[0] . '') . '
                                                       </div>
                                                    </a>
                                                    <div class="oxi-addons-shortcode-import-bottom">
                                                        <span>' . oxi_addons_shortcode_name_converter($val[0]) . '</span>
                                                        <input type="button" class="btn btn-outline-info btn-sm text-right OxiElementsADD" name="OxiElementsADD" OXIAddonsElements="' . $val[0] . '" value="Install">
                                                    </div>
                                                </div>';
                    }
                }
            ...
            <div class="oxi-addons-wrapper">
                ...
                foreach ($elements[1] as $key => $element) {
                    foreach ($element as $val) {
                        $valuetrue = '';
                        foreach ($customaddons as $customelements) {
                            if ($customelements['name'] == $val[0]) {
                                $valuetrue = 'true';
                                $customvr = $customelements['version'];
                                $customvr = $customelements['version'];
                            }
                        }
                        if ($valuetrue == 'true' && (version_compare(((float) $val[1]), $customvr, '>'))) {
                            //echo 'delete bosbe';
                            echo '<div class="oxi-addons-shortcode-import"  oxi-addons-search="' . $val[0] . '" id ="' . $val[0] . '">
                                        <a target="_blank" href="https://www.shortcode-addons.com/' . str_replace('_', '-', $val[0]) . '">
                                           <div class="oxi-addons-shortcode-import-top">
                                                  ' . oxi_addons_admin_font_awesome('oxi-' . $val[0] . '') . '
                                           </div>
                                        </a>
                                        <div class="oxi-addons-shortcode-import-bottom">
                                            <span>' . oxi_addons_shortcode_name_converter($val[0]) . '</span>
                                            <input type="button" class="btn btn-outline-info btn-sm text-right OxiElementsADD" name="OxiElementsADD" oxi-custom-addons="yes" OXIAddonsElements="' . $val[0] . '" value="UPDATE">
                                        </div>
                                    </div>';
                        } else if ($valuetrue == 'true' && (version_compare((float) $val[1], $customvr, '<='))) {
                            //echo 'update bosbe';
                            echo '<div class="oxi-addons-shortcode-import"  oxi-addons-search="' . $val[0] . '" id ="' . $val[0] . '">
                                        <a target="_blank" href="https://www.shortcode-addons.com/' . str_replace('_', '-', $val[0]) . '">
                                           <div class="oxi-addons-shortcode-import-top">
                                        ...
                                        <form method="post" class="OXIAddonsElementsDeleteSubmit">
                                            <div class="oxi-addons-shortcode-import-bottom">
                                                <span>' . oxi_addons_shortcode_name_converter($val[0]) . '</span>
                                                <input type="hidden" id="OXIAddonsElements" name="OXIAddonsElements" value="' . $val[0] . '">
                                                <input type="submit" class="btn btn-outline-warning btn-sm text-right OxiElementsADD oxi-addons-addons-element-btn-warning " name="OxiAddonsElementsDLT" value="Delete">
                                                ' . wp_nonce_field('Oxi-Addons-Delete-addons-Nonce') . '
                                            </div> 
                                        </form>';
                        } else {
                            echo '<div class="oxi-addons-shortcode-import"  oxi-addons-search="' . $val[0] . '" id ="' . $val[0] . '">
                                        ...
                                        <div class="oxi-addons-shortcode-import-bottom">
                                            <span>' . oxi_addons_shortcode_name_converter($val[0]) . '</span>
                                            <input type="button" class="btn btn-outline-info btn-sm text-right OxiElementsADD" name="OxiElementsADD"  oxi-custom-addons="yes" OXIAddonsElements="' . $val[0] . '" value="Install">
                                        </div>
                                    </div>';
                        }
                    }
