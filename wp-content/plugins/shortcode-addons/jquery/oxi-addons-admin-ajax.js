jQuery(function () {
    ...
    if (jQuery('#OxiAddonsElementsData').length > 0) {
        ...
        var addons = 'elementor_templates, accordion,banner,button,carousel,content_boxes,count_down,counter,divider,drop_caps,event_widgets,food_menu,footer_info,headers,heading,icon,icon_boxes,icon_effects,image_boxes,info_banner,info_boxes,info_image_boxes,link_effects,logo_showcase,price_table,single_image,social_icons,spacer,team,text_blocks,testimonial';
        var addonsdata = addons.split(',');
        var custom = '';
        addonsdata.forEach(function (item, index) {
            if(index === 'elementor_templates'){
                var custom = 'custom_check';
            }
            setTimeout(function () {
                jQuery.post({
                    url: OxiAddonsAdminajax.ajaxurl,
                    ...
                    success: function (data) {
                        ...
                        jQuery(".oxi-addons-pei-chart-wrap").css({width: datawidth + "%"}, 500);
                        if (datawidth < 100) {
    ...
    jQuery(".oxi-addons-shortcode-import-bottom .btn-outline-info.OxiElementsADD").on("click", function (e) {
        ...
        var custom = jQuery(this).attr("oxi-custom-addons");
