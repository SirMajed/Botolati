...
function um_custom_wp_err_messages( $message ) {
	...
	if ( isset( $_REQUEST['err'] ) && !empty( $_REQUEST['err'] ) ) {
		switch( $_REQUEST['err'] ) {
			case 'blocked_email':
				$err = __('. تم حظر هذا البريد الالكتروني','ultimate-member');
				break;
			case 'blocked_ip':
				$err = __('. تم حظر الآي بي الخاص بك','ultimate-member');
				break;
		}
	}
...
function um_wp_form_errors_hook_logincheck( $user, $username, $password ) {
	...
	if ( isset( $user->ID ) ) {
		...
		switch( $status ) {
			case 'inactive':
				return new WP_Error( $status, __('. تم تعطيل حسابك','ultimate-member') );
				break;
			case 'awaiting_admin_review':
				return new WP_Error( $status, __('. لم يتم قبول حسابك حتى الآن','ultimate-member') );
				break;
			case 'awaiting_email_confirmation':
				return new WP_Error( $status, __('. الرجاء تفعيل حسابك عبر بريدك الالكتروني','ultimate-member') );
				break;
			case 'rejected':
