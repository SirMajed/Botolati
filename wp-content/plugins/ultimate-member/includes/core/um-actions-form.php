	...
	if ( ! empty( $fields ) ) {
		foreach ( $fields as $key => $array ) {
			...
			if ( isset( $array['type'] ) && $array['type'] == 'checkbox' && isset( $array['required'] ) && $array['required'] == 1 && !isset( $args[$key] ) ) {
				UM()->form()->add_error($key, sprintf(__('%s متطلب.','ultimate-member'), $array['title'] ) );
			}
			...
			if ( isset( $args[ $key ] ) ) {
				...
				if ( isset( $array['required'] ) && $array['required'] == 1 ) {
					if ( ! isset( $args[$key] ) || $args[$key] == '' || $args[$key] == 'empty_file') {
						if( empty( $array['label'] ) ) {
							UM()->form()->add_error($key, __('هذا الحقل متطلب','ultimate-member') );
						} else {
							UM()->form()->add_error($key, sprintf( __('%s متطلب','ultimate-member'), $array['label'] ) );
						}
					}
				}
				...
				if ( isset( $array['force_good_pass'] ) && $array['force_good_pass'] == 1 ) {
					if ( ! UM()->validation()->strong_pass( $args[$key] ) ) {
						UM()->form()->add_error($key, __('رقمك السري يجب ان يحتوي على الاقل حرف واحد صغير وحرف واحد كبير ورقم','ultimate-member') );
					}
				...
				if ( isset( $array['force_confirm_pass'] ) && $array['force_confirm_pass'] == 1 ) {
					if ( $args[ 'confirm_' . $key] == '' && ! UM()->form()->has_error($key) ) {
						UM()->form()->add_error( 'confirm_' . $key , __('تأكيد الرقم السري','ultimate-member') );
					}
					if ( $args[ 'confirm_' . $key] != $args[$key] && !UM()->form()->has_error($key) ) {
						UM()->form()->add_error( 'confirm_' . $key , __('الرقم السري لا يتطابق مع المدخل السابق','ultimate-member') );
					}
				}
					...
					switch( $array['validate'] ) {
						...
						case 'numeric':
							if ( $args[$key] && !is_numeric( $args[$key] ) ) {
								UM()->form()->add_error($key, __('هذا الحقل يتطلب ادخال ارقام فقط','ultimate-member') );
							}
							break;
						...
						case 'unique_username':
							...
							if ( $args[ $key ] == '' ) {
								UM()->form()->add_error( $key, __( 'يجب عليك كتابة اسم المستخدم', 'ultimate-member' ) );
							} elseif ( $mode == 'register' && username_exists( sanitize_user( $args[ $key ] ) ) ) {
								UM()->form()->add_error( $key, __( 'اسم المستخدم مستخدم من قبل', 'ultimate-member' ) );
							} elseif ( is_email( $args[ $key ] ) ) {
								UM()->form()->add_error( $key, __( 'لايمكنك كتابة بريد الكتروني في هذا الحقل', 'ultimate-member' ) );
							} elseif ( ! UM()->validation()->safe_username( $args[$key] ) ) {
								UM()->form()->add_error( $key, __( 'اسم المستخدم يحتوي على حروف خاطئة', 'ultimate-member' ) );
							}
						...
						case 'unique_username_or_email':
							...
							if ( $args[$key] == '' ) {
								...
								UM()->form()->add_error($key,  __('هذا البريد الالكتروني مسجل من قبل','ultimate-member') );
						...
						case 'unique_email':
							...
							if ( in_array( $key, array('user_email') ) ) {
								...
								if ( $args[ $key ] == '' && in_array( $key, array('user_email') ) ) {
									UM()->form()->add_error( $key, __('يجب عليك كتابة البريد الالكتروني','ultimate-member') );
								} else if ( in_array( $mode, array('register') )  && $email_exists  ) {
									UM()->form()->add_error($key, __('هذا البريد الالكتروني مستخدم من قبل','ultimate-member') );
								} else if ( in_array( $mode, array('profile') )  && $email_exists && $email_exists != $args['user_id']  ) {
									UM()->form()->add_error( $key, __('هذا البريد الالكتروني مستخدم من قبل','ultimate-member') );
								} else if ( !is_email( $args[ $key ] ) ) {
									UM()->form()->add_error( $key, __('البريد الالكتروني الذي ادخلته غير صالح','ultimate-member') );
								} else if ( ! UM()->validation()->safe_username( $args[ $key ] ) ) {
									UM()->form()->add_error( $key,  __('الرجاء كتابة البريد الالكتروني بشكل صحيح','ultimate-member') );
								}
			...
			if ( isset( $args['description'] ) ) {
				...
				if( $profile_show_bio ){
					if ( strlen( utf8_decode( $args['description'] ) ) > $max_chars && $max_chars  ) {
						UM()->form()->add_error('description', sprintf(__('يجب عليك كتابة اقل من  %s كلمة','ultimate-member'), $max_chars ) );
					}
				}
