		...
		<div class="um-search um-search-<?php echo $count; ?>">
			...
			<form method="get" action="">
				...
				<div class="um-search-submit">
					...
					<a href="#" class="um-button um-do-search"><?php _e('بحث','ultimate-member'); ?></a>
					<a href="<?php echo UM()->permalinks()->get_current_url( true ); ?>" class="um-button um-alt"><?php _e('اعادة','ultimate-member'); ?></a>
...
function um_members_directory_pagination( $args ) {
	...
	if ( um_members('total_pages') > 1 ) { // needs pagination
        ...
        <div class="um-members-pagidrop uimob340-show uimob500-show">
			...
			<?php _e(': اذهب لصفحة','ultimate-member'); ?>
        ...
        <div class="um-members-pagi uimob340-hide uimob500-hide">
			...
			<?php if ( um_members('page') != 1 ) { ?>
                <a href="<?php echo UM()->permalinks()->add_query( 'members_page', 1 ); ?>" class="pagi pagi-arrow um-tip-n" title="<?php _e('الصفحة الاولى','ultimate-member'); ?>"><i class="um-faicon-angle-double-left"></i></a>
			<?php } else { ?>
                <span class="pagi pagi-arrow disabled"><i class="um-faicon-angle-double-left"></i></span>
			<?php } ?>

			<?php if ( um_members('page') > 1 ) { ?>
                <a href="<?php echo UM()->permalinks()->add_query( 'members_page', um_members('page') - 1 ); ?>" class="pagi pagi-arrow um-tip-n" title="<?php _e('السابق','ultimate-member'); ?>"><i class="um-faicon-angle-left"></i></a>
			<?php } else { ?>
			...
			<?php if ( um_members('page') != um_members('total_pages') ) { ?>
                <a href="<?php echo UM()->permalinks()->add_query( 'members_page', um_members('page') + 1 ); ?>" class="pagi pagi-arrow um-tip-n" title="<?php _e('التالي','ultimate-member'); ?>"><i class="um-faicon-angle-right"></i></a>
			<?php } else { ?>
                <span class="pagi pagi-arrow disabled"><i class="um-faicon-angle-right"></i></span>
			<?php } ?>

			<?php if ( um_members('page') != um_members('total_pages') ) { ?>
                <a href="<?php echo UM()->permalinks()->add_query( 'members_page', um_members('total_pages') ); ?>" class="pagi pagi-arrow um-tip-n" title="<?php _e('الصفحة الاخيرة','ultimate-member'); ?>"><i class="um-faicon-angle-double-right"></i></a>
			<?php } else { ?>
