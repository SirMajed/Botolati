...
function um_submit_form_errors_hook_login( $args ) {
	...
	if ( isset( $args['username'] ) && $args['username'] == '' ) {
		UM()->form()->add_error( 'username',  __('الرجاء ادخال اسم المستخدم او البريد الالكتروني','ultimate-member') );
	}

	if ( isset( $args['user_login'] ) && $args['user_login'] == '' ) {
		UM()->form()->add_error( 'user_login',  __('الرجاء ادخال اسم المستخدم','ultimate-member') );
	}

	if ( isset( $args['user_email'] ) && $args['user_email'] == '' ) {
		UM()->form()->add_error( 'user_email',  __('الرجاء ادخال البريد الالكتروني','ultimate-member') );
	}
	...
	if ( $args['user_password'] == '' ) {
		UM()->form()->add_error( 'user_password',  __('الرجاء ادخل الرقم السري','ultimate-member') );
	}
	...
	if ( $user && wp_check_password( $args['user_password'], $user->data->user_pass, $user->ID) ) {
		...
		UM()->form()->add_error( 'user_password',  __('.الرقم السري غير صحيح','ultimate-member') );
	}
...
function um_add_submit_button_to_login( $args ) {
	...
	<div class="um-col-alt">
		...
		<?php if ( isset( $args['show_rememberme'] ) && $args['show_rememberme'] ) {
			UM()->fields()->checkbox( 'rememberme', __( 'اجعلني متصلا', 'ultimate-member' ), false ); ?>
...
function um_after_login_submit( $args ) {
	...
	<div class="um-col-alt-b">
		<a href="<?php echo um_get_core_page('password-reset'); ?>" class="um-link-alt">
			<?php _e( 'نسيت الرقم السري ؟ ', 'ultimate-member' ); ?>
		</a>
	</div>
