	...
	class Fields {
		...
		function get_field( $key ) {
			...
			switch ($array['type']) {
				...
				case 'image':
					...
					if (!isset( $array['invalid_image'] )) $array['invalid_image'] = __( "الرجاء رفع صورة اخرى", 'ultimate-member' );
					...
					if (!isset( $array['button_text'] )) $array['button_text'] = __( 'رفع', 'ultimate-member' );
					if (!isset( $array['extension_error'] )) $array['extension_error'] = __( ". نعتذر هذه الصورة لايمكن رفعها الرجاء رفع صورة اخرى", 'ultimate-member' );
					if (!isset( $array['max_size_error'] )) $array['max_size_error'] = __( "حجم الصورة كبير جدا", 'ultimate-member' );
					if (!isset( $array['min_size_error'] )) $array['min_size_error'] = __( "الصورة صغيرة جدا", 'ultimate-member' );
					if (!isset( $array['max_files_error'] )) $array['max_files_error'] = __( "يمكنك فقط رفع صورة واحدة", 'ultimate-member' );
		...
		function edit_field( $key, $data, $rule = false, $args = array() ) {
			...
			switch ( $type ) {
				...
				case 'password':
					...
					if ($key == 'single_user_password') {
						...
						if ($this->set_mode == 'account' || um_is_core_page( 'account' )) {
							...
							if (isset( $data['label'] )) {
								$output .= $this->field_label( __( 'الرقم السري الحالي', 'ultimate-member' ), $key, $data );
							}
						...
						if ($this->set_mode == 'account' && um_is_core_page( 'account' ) || $this->set_mode == 'password' && um_is_core_page( 'password-reset' )) {
							...
							$output .= $this->field_label( __( 'الرقم السري الجديد', 'ultimate-member' ), $key, $data );
						...
						if ($this->set_mode != 'login' && isset( $data['force_confirm_pass'] ) && $data['force_confirm_pass'] == 1) {
							...
							if (isset( $data['label'] )) {
								$output .= $this->field_label( sprintf( __( 'اعد كتابة الرقم السري', 'ultimate-member' ), $data['label'] ), $key, $data );
							}
				...
				case 'image':
					...
					$modal_label = ( isset( $data['label'] ) ) ? $data['label'] : __( 'رفع صورة جديدة', 'ultimate-member' );
					...
					if ( $field_value ) {
						...
						$output .= '<div class="um-single-image-preview show ' . $crop_class . '" data-crop="' . $crop_data . '" data-key="' . $key . '">
                            ...
                            </div><a href="javascript:void(0);" data-modal="um_upload_single" data-modal-size="' . $modal_size . '" data-modal-copy="1" class="um-button um-btn-auto-width">' . __( 'تغيير الصورة', 'ultimate-member' ) . '</a>';
					} else {
					...
					$output .= '<div class="um-modal-footer">
                                    <div class="um-modal-right">
                                        <a href="javascript:void(0);" class="um-modal-btn um-finish-upload image disabled" data-key="' . $key . '" data-change="' . __( 'تغيير الصورة', 'ultimate-member' ) . '" data-processing="' . __( 'جاري المعالجة ...', 'ultimate-member' ) . '"> ' . __( 'تأكيد', 'ultimate-member' ) . '</a>
                                        <a href="javascript:void(0);" class="um-modal-btn alt" data-action="um_remove_modal"> ' . __( 'الغاء', 'ultimate-member' ) . '</a>
                                    </div>
				...
				case 'file':
					...
					$modal_label = ( isset( $data['label'] ) ) ? $data['label'] : __( 'رفع ملف', 'ultimate-member' );
					$output .= '<div class="um-field-area" style="text-align: center;">';
		...
		function display_view( $mode, $args ) {
			...
			if ( UM()->options()->get( 'profile_empty_text' ) ) {
				...
				if (um_is_myprofile()) {
					$output .= '<p class="um-profile-note">' . $emo . '<span>' . sprintf( __( 'حسابك يبدو انه فارغ قليلا , لماذا لاتضع نبذة بسيطة عنك ؟  <a href="%s">اضافة نبذة</a>', 'ultimate-member' ), um_edit_profile_url() ) . '</span></p>';
				} else {
					$output .= '<p class="um-profile-note">' . $emo . '<span>' . __( '. هذا المستخدم لايوجد لديه نبذة ', 'ultimate-member' ) . '</span></p>';
				}
			}
