...
if ( ! class_exists( 'um\core\Password' ) ) {
	...
	class Password {
		...
		function um_reset_password_errors_hook( $args ) {
			...
			if ( empty( $user ) ) {
				UM()->form()->add_error('username_b', __( 'الرجاء ادخال البريد الالكتروني او اسم المستخدم', 'ultimate-member' ) );
			}

			if ( ( ! is_email( $user ) && ! username_exists( $user ) ) || ( is_email( $user ) && ! email_exists( $user ) ) ) {
				UM()->form()->add_error('username_b', __( 'لايوجد حساب مسجل بهذا الاسم او البريد الالكتروني','ultimate-member') );
			} else {
				...
				if ( UM()->options()->get( 'enable_reset_password_limit' ) ) { // if reset password limit is set
					...
					if ( UM()->options()->get( 'disable_admin_reset_password_limit' ) &&  $is_admin ) {
						...
						if ( $attempts >= $limit ) {
							UM()->form()->add_error( 'username_b', __( 'لقد استخدمت محاولاتك الكاملة , الرجاء التواصل مع الدعم الفني من خلال صفحة اتصل بنا','ultimate-member') );
						} else {
		...
		function um_change_password_errors_hook( $args ) {
			...
			if ( isset( $args['user_password'] ) && empty( $args['user_password'] ) ) {
				UM()->form()->add_error('user_password', __('الرجاء ادخال رقم سري جديد','ultimate-member') );
			}
			...
			if ( isset( $args['confirm_user_password'] ) && empty( $args['confirm_user_password'] ) ) {
				UM()->form()->add_error('confirm_user_password', __('الرجاء تأكيد الرقم السري الجديد','ultimate-member') );
			}

			if ( isset( $args['user_password'] ) && isset( $args['confirm_user_password'] ) && $args['user_password'] != $args['confirm_user_password'] ) {
				UM()->form()->add_error('confirm_user_password', __('الرقم السري الجديد لايتطابق مع المدخل السابق','ultimate-member') );
			}
