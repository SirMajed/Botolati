...
function um_submit_account_errors_hook( $args ) {
	...
	if ( ! wp_verify_nonce( $_POST[ 'um_account_nonce_' . $_POST['_um_account_tab'] ], 'um_update_account_' . $_POST['_um_account_tab'] ) ) {
		UM()->form()->add_error('um_account_security', __( 'هل انت هكر ؟ الرجاء حاول مرة اخرى', 'ultimate-member' ) );
	}
		...
		switch ( $_POST['_um_account_tab'] ) {
			case 'delete': {
				...
				if ( strlen(trim( $_POST['single_user_password'] ) ) == 0 ) {
					UM()->form()->add_error( 'single_user_password', __( 'يجب عليك ادخال الرقم السري', 'ultimate-member' ) );
				} else {
					if ( ! wp_check_password( $_POST['single_user_password'], $user->data->user_pass, $user->data->ID ) ) {
						UM()->form()->add_error( 'single_user_password', __( 'الرقم السري خاطئ', 'ultimate-member' ) );
					}
				}
			...
			case 'password': {
				...
				if ( ( isset( $_POST['current_user_password'] ) && $_POST['current_user_password'] != '' ) ||
					...
					if ( $_POST['current_user_password'] == '' || ! wp_check_password( $_POST['current_user_password'], $user->data->user_pass, $user->data->ID ) ) {
						...
						UM()->form()->add_error('current_user_password', __('الرقم السري خاطئ','ultimate-member') );
						UM()->account()->current_tab = 'password';
					} else { // correct password

						if ( $_POST['user_password'] != $_POST['confirm_user_password'] && $_POST['user_password'] ) {
							UM()->form()->add_error('user_password', __('الرقم السري الذي ادخلته لايتطابق من المدخل السابق','ultimate-member') );
							UM()->account()->current_tab = 'password';
						}
						...
						if ( UM()->options()->get( 'account_require_strongpass' ) ) {
							...
							if ( strlen( utf8_decode( $_POST['user_password'] ) ) < 8 ) {
								UM()->form()->add_error('user_password', __('الرقم السري يجب ان يكون ثمانية خانات واكثر','ultimate-member') );
							}

							if ( strlen( utf8_decode( $_POST['user_password'] ) ) > 30 ) {
								UM()->form()->add_error('user_password', __('الرقم السري لايجب ان يكون اطول من 30 خانة','ultimate-member') );
							}

							if ( ! UM()->validation()->strong_pass( $_POST['user_password'] ) ) {
								UM()->form()->add_error('user_password', __('الرقم السري يجب ان يتكون من حرف صغير وحرف كبير ورقم','ultimate-member') );
							}
			...
			case 'general': {
				...
				if ( ! empty( $_POST['user_login'] ) && ! validate_username( $_POST['user_login'] ) ) {
					UM()->form()->add_error('user_login', __( 'اسم المستخدم غير صحيح', 'ultimate-member' ) );
					return;
				}

				if ( isset( $_POST['first_name'] ) && ( strlen( trim( $_POST['first_name'] ) ) == 0 && $account_name_require ) ) {
					UM()->form()->add_error( 'first_name', __( 'يجب عليك كتابة اسمك الاول', 'ultimate-member' ) );
				}

				if ( isset( $_POST['last_name'] ) && ( strlen( trim( $_POST['last_name'] ) ) == 0 && $account_name_require ) ) {
					UM()->form()->add_error( 'last_name', __( 'يجب عليك كتابة الاسم الاخير', 'ultimate-member' ) );
				}

				if ( isset( $_POST['user_email'] ) ) {

					if ( strlen( trim( $_POST['user_email'] ) ) == 0 ) {
						UM()->form()->add_error( 'user_email', __( 'يجب عليك كتابة البريد الالكتروني', 'ultimate-member' ) );
					}

					if ( ! is_email( $_POST['user_email'] ) ) {
						UM()->form()->add_error( 'user_email', __( 'الرجاء كتابة بريد الكتروني بشكل صحيح', 'ultimate-member' ) );
					}

					if ( email_exists( $_POST['user_email'] ) && email_exists( $_POST['user_email'] ) != get_current_user_id() ) {
						UM()->form()->add_error( 'user_email', __( 'البريد الالكتروني مستخدم من قبل', 'ultimate-member' ) );
					}
				}
				...
				if ( UM()->options()->get( 'account_general_password' ) ) {
					if ( strlen( trim( $_POST['single_user_password'] ) ) == 0 ) {
						UM()->form()->add_error('single_user_password', __( 'الرجاء ادخال رقمك السري', 'ultimate-member' ) );
					} else {
						if ( ! wp_check_password( $_POST['single_user_password'], $user->data->user_pass, $user->data->ID ) ) {
							UM()->form()->add_error('single_user_password', __( 'الرقم السري غير صحيح', 'ultimate-member' ) );
						}
					}
				}
...
function um_submit_account_details( $args ) {
	...
	if ( isset( $changes['hide_in_members'] ) && ( $changes['hide_in_members'] == __('لا','ultimate-member') || $changes['hide_in_members'] == 'No' ) ) {
